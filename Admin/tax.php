                ...
                <div class="table-responsive">
                    <table class="table table-striped custom-table mb-0">
                        ...
                        <tbody>
                            ...
                            // Get the user_id from the session (assuming it's stored in session)
                            $user_id = $_SESSION['user_id'];

                            // Fetch taxes data from the database
                            $sql = "SELECT * FROM taxes WHERE user_id = ?";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param("i", $user_id);
                            $stmt->execute();
                            $result = $stmt->get_result();

                            // Check if there are any taxes
                            if ($result->num_rows > 0) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo '<td>' . $row['tax_percentage'] . '</td>';
                                    echo '<td>';
                                    echo '<div class="dropdown action-label">';
                                    if ($row['status'] == 'Active') {
                                        echo '<a class="custom-badge status-green dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">Active</a>';
                                    } else {
                                        echo '<a class="custom-badge status-red dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">Inactive</a>';
                                    }
                                    echo '<div class="dropdown-menu">';
                                    echo '<a class="dropdown-item" href="#">Active</a>';
                                    echo '<a class="dropdown-item" href="#">Inactive</a>';
                                    echo '</div>';
                                    echo '</div>';
                                    echo '</td>';
                                    ...
                                    echo '<a class="dropdown-item" href="edit-tax.html"><i class="fa fa-pencil m-r-5"></i> Edit</a>';
                                    echo '<a class="dropdown-item" href="#" data-toggle="modal" data-target="#delete_tax"><i class="fa fa-trash-o m-r-5"></i> Delete</a>';
                                    echo '</div>';
                                    echo '</div>';
                                    echo '</td>';
                                    echo '</tr>';
