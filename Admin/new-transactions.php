...
if ($result->num_rows > 0) {
    ...
    $last_transaction_id = $row["max_id"];
    // Increment the last transaction number
    $next_transaction_id = $last_transaction_id + 1;
} else {
    // If no transaction exists yet, start from 1
    $next_transaction_id = 1;
}

// Format the transaction number with leading zeros
$transaction_no = "TRXN-" . str_pad($next_transaction_id, 3, "0", STR_PAD_LEFT);

if (isset($_POST['record_transaction'])) {
    // Get form data
    $transaction_no = $_POST['transaction_no'];
    $transaction_date = $_POST['transaction_date'];
    $transaction_type = $_POST['transactionType']; // Correct the name attribute to match the form field
    $amount = $_POST['amount'];
    ...
    $sql = "INSERT INTO transactions (transaction_number, transaction_date, transaction_type, amount, description, customer_name, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)";

    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters
        $stmt->bind_param("sssdssi", $transaction_no, $transaction_date, $transaction_type, $amount, $description, $customer_name, $user_id);

        // Attempt to execute the prepared statement
        if ($stmt->execute()) {
            echo "Records inserted successfully.";
            header('Location: all-transactions.php');
        } else {
...
<div class="page-wrapper">
    ...
    <input class="form-control" type="text" name="transaction_no" value="<?php echo $transaction_no; ?>" placeholder="Enter transaction ID" readonly>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label>Amount</label>
                                <input class="form-control" type="text" name="amount" placeholder="Enter transaction amount">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label>Date</label>
                                <input class="form-control datetimepicker" name="transaction_date" placeholder="Enter transaction date">
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="description" placeholder="Enter transaction description">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="transactionType" id="credit" value="credit" checked>
    ...
    <div class="row">
        <div class="col text-center">
            <button class="btn btn-primary submit-btn" name="record_transaction" style="width: 120px;">Record Transaction</button>
        </div>
        <div class="col text-center">
            <button class="btn btn-success"><a href="proceed-trans.php" style="color: white; text-decoration: none;">Proceed to transactions</a></button>
        </div>
    </div>
</div>
