...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_payment'])) {
    ...
    // Prepare and execute SQL query to update payment details
    $sql = "UPDATE payments SET invoice_number=?, amount=?, payment_date=?, customer_id=?, status=?, description=? WHERE payment_id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssi", $invoice_number, $amount, $payment_date, $customer_id, $status, $description, $payment_id);

    // Execute the statement
    if ($stmt->execute()) {
        // Payment successfully updated
        header('Location: payments.php'); // Redirect to payments page
        exit();
    } else {
        // Failed to update payment
        $error_msg = "Error: Unable to update payment. Please try again.";
    }
        ...
        <form action="edit-payment.php" method="POST">
            ...
            <div class="row">
                        ...
                        <select class="form-control select" name="invoice_number">
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    $selected = ($row['invoice_number'] == $payment['invoice_number']) ? 'selected' : '';
                                    echo "<option value='" . $row['invoice_number'] . "' $selected>" . $row['invoice_number'] . "</option>";
                                }
                            } else {
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="pay_amount" placeholder="Enter payment amount" value="<?php echo $payment['amount']; ?>">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="date" name="pay_date" placeholder="Enter payment date" value="<?php echo $payment['payment_date']; ?>">
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="form-group">
                        <label>Description</label>
                        <textarea class="form-control" name="description" placeholder="Enter description"><?php echo $payment['description']; ?></textarea>
                    </div>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="form-group">
                    ...
                    <select class="form-control select" name="customer_id">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                $selected = ($row['customer_id'] == $payment['customer_id']) ? 'selected' : '';
                                echo "<option value='" . $row['customer_id'] . "' $selected>" . $row['customer_id'] . "</option>";
                            }
                        } else {
            ...
            <div class="form-group">
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="status" id="customer_active" value="Completed" <?php echo ($payment['status'] == 'Completed') ? 'checked' : ''; ?>>
                    <label class="form-check-label" for="customer_active">Completed</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="status" id="customer_inactive" value="Pending" <?php echo ($payment['status'] == 'Pending') ? 'checked' : ''; ?>>
                    <label class="form-check-label" for="customer_inactive">Pending</label>
                </div>
            </div>
            <div class="form-group text-right">
                <button type="submit" class="btn btn-primary" name = "edit_payment" >Update Payment</button>
            </div>
        </form>
    </div>
</div>
